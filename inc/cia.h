...
#define CIA1_A_IRQ 0x0010
#define CIA1_B_IRQ 0x0020

#define CIA2_A_NIM 0x0100
#define CIA2_B_NIM 0x0200
...
typedef struct {
    ...
    uint8_t icr;  // Interrupt Control Register (ICR)
                  ...
                  // Bit 3: 1 = Interruptfreigabe für das Ende der Übertragung eines kompletten Bytes über das serielle
                  // Schieberegister. Bit 4: 1 = Interruptfreigabe für das Erkennen einer negativen Flanke am FLAG-Pin.
    uint8_t cra;  // Control Register A (CRA)
                  ...
                  // Bit 2: 0 = Bei Timer Unterlauf wird an Port B das Bit 6 invertiert , 1 = Bei Timer-Unterlauf wird
                  // an Port B das Bit 6 für einen Systemtaktzyklus High Bit 3: 0 = Timer-Neustart nach Unterlauf (Latch
                  // wird neu geladen), 1 = Timer stoppt nach Unterlauf Bit 4: 1 = Einmalig Latch in den Timer laden Bit
                  // 5: 0 = Timer wird mit der Systemfrequenz getaktet, 1 = Timer wird von positiven Flanken am CNT-Pin
                  // getaktet Bit 6: Richtung des seriellen Schieberegisters, 0 = SP-Pin ist Eingang (lesen), 1 = SP-Pin
                  // ist Ausgang (schreiben) Bit 7: Echtzeituhr, 0 = 60 Hz, 1 = 50 Hz an Pin 19
    ...
    uint8_t crb;  // Control Register B (CRB)
                  ...
                  // Bit 2: 0 = Bei Timer Unterlauf wird an Port B das Bit 7 invertiert , 1 = Bei Timer-Unterlauf wird
                  // an Port B das Bit 7 für einen Systemtaktzyklus High Bit 3: 0 = Timer-Neustart nach Unterlauf (Latch
                  // wird neu geladen), 1 = Timer stoppt nach Unterlauf Bit 4: 1 = Einmalig Latch in den Timer laden
    ...
    uint16_t reloadTimerA;
    uint16_t reloadTimerB;
} CIA;
...
extern void updateCia1(uint8_t clkCount);
extern void updateCia2(uint8_t clkCount);
extern void writeCia1toTxtFile(void);
extern void writeCia2toTxtFile(void);
extern int writeCiatoFile(const char*, CIA* cia, RTC_CIA* ciaRTC);
