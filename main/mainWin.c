...
void terminateWindow(void) { SendMessage(windowHandle, WM_CLOSE, 0, 0); }
    ...
    switch (uMsg) {
        ...
        case WM_PAINT:
            ...
            if (canvas) {
                ...
                //            SetDIBitsToDevice(hdc, 0, 0,  PAL_B_X, PAL_B_Y, 0, 0, 0, PAL_B_Y, canvas, bmi,
                //            DIB_RGB_COLORS);
            }
            EndPaint(hwnd, &ps);
            break;
...
int runMainWindow(void) {
    ...
    AdjustWindowRectEx(&windowRect,          // RECT, der die gewünschte Größe und Position enthält
    ...
    HWND hwnd = CreateWindowEx(0,                              // Erweiterter Fensterstil
